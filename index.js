...
var parse_route = function (infile, outfile) {
  ...
  var read_stream = fs.createReadStream(infile).pipe(es.split()).pipe(es.mapSync(line => {
    read_stream.pause();
    ln++;
    if (ln % 100000 == 0) console.log('line: ', ln);
  ...
  return new Promise((res, rej) => {
    read_stream.on('error', err => res(routes)); // reslove anyway.
    read_stream.on('end', () => res(routes));
  });
};

parse_route(args[0]).then(routes => {
  var ws = fs.createWriteStream(args[1]);
  ws.write(JSON.stringify(routes));
  ws.end();
});
